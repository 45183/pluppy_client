...
//     try {
//       const res = await axiosInstance.get(`/product/category/${subCategoryId}`);
//       if (Array.isArray(res.data.data)) {
//         setProducts(res.data.data);
//         console.log(res.data.data[0].image1);
//       } else {
...
function Product() {
  ...
  const handleMainCategoryClick = async (mainCategory) => {
    try {
      const res = await axiosInstance.get(`/product/mainCategory/${mainCategory.mainCategoryId}`);
      if (Array.isArray(res.data.data)) {
        setProducts(res.data.data);
      } else {
        console.error('Invalid response format:', res.data);
  ...
  const handleSubCategoryClick = async (subCategory) => {
    try {
      const res = await axiosInstance.get(`/product/subCategory/${subCategory.subCategoryId}`);
      if (Array.isArray(res.data.data)) {
        setProducts(res.data.data);
      } else {
        console.error('Invalid response format:', res.data);
      ...
      <div className="category">
        <div className='main_category'>
          ...
          {categories.map((category) => (
            <button key={category.mainCategoryId} onClick={() => handleMainCategoryClick(category)}>
              {category.name}
            </button>
          ))}
        </div>
